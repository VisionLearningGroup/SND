...
class ResNetFc(nn.Module):
  ...
  def forward(self, x, nobot=False):
...
class ResNetFc_Semi(nn.Module):
  def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    super(ResNetFc_Semi, self).__init__()
    model_resnet = resnet_dict[resnet_name](pretrained=True)
    self.conv1 = model_resnet.conv1
    self.bn1 = model_resnet.bn1
    self.relu = model_resnet.relu
  ...
  def forward(self, x,target=False):
    ...
    y = self.fc(F.normalize(x))/0.05
    if not target:
        return x, y
    else:
        y2 = self.fc(F.normalize(grad_reverse(x))) / 0.05
        return x,y,y2
  def output_num(self):
    return self.__in_features
